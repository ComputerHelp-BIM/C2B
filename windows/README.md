...
Put your own template at `templates\CH-TEMPLATE.dxf` and `C2B-run.bat` will use
it as the
seed automatically (layers, text styles, dimension style and legend are taken
from it).

Make a desktop shortcut to `C2B.bat` for each drafter: right-click it, Send to,
Desktop.

These are convenience wrappers. Everything they do is available from a command
prompt with
