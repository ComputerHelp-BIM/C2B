...
def write_normalized_workbook(np_: NormalizedProject, path: str | Path) -> Path:
    ...
    _sheet(wb, "Slabs", ["Id", "Floor", "Kind", "Mark", "Thickness", "Source", "Area (m2)", "Centroid X", "Centroid Y", "Top offset (mm)", "Offset rule", "Support depth", "Sunk", "Sunk source", "Holes", "Openings", "Tags used"],
           [[p.id, p.floor_id, p.kind, p.mark, p.thickness_mm, p.thickness_source, p.area_m2, p.centroid.x, p.centroid.y, p.top_offset_mm, p.top_offset_rule, p.support_depth_mm, p.sunk_mm, p.sunk_source, len(p.holes), p.opening_ids, p.tag_ids] for p in np_.panels])
